...
<c:if test="${not empty user}">
    <c:set var="errorReg" value="${user.errorInformant}"/>
</c:if>
    ...
    <div id="header">
        ...
        <div id="menubar">
            <ul id="menu">
                ...
                <li><c:choose>
                    <c:when test="${sessionScope.SESSION_USER.role eq 0}">
                        <a href="user-home.jsp">Home</a></c:when>
                    <c:when test="${sessionScope.SESSION_USER.role eq 1}">
                        <a href="tech-home.jsp">Home</a></c:when>
                    <c:when test="${sessionScope.SESSION_USER.role eq 2}">
                        <a href="admin-home.jsp">Home</a></c:when>
                </c:choose></li>
            </ul>
        </div>
    </div>
    <div id="site_content">
        <form id="updateForm" action="MainController" method="post" style="margin-bottom: 0">
            <table>
                ...
                <tr>
                    <th>Given Name</th>
                    <td><input type="text" name="givenName" value="${user.givenName}"/></td>
                    <td>
                ...
                <tr>
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${user.role eq 0}">User</c:when>
                            <c:when test="${user.role eq 1}">Tech</c:when>
                            <c:when test="${user.role eq 2}">Admin</c:when>
                        </c:choose>
                    </td>
                    <td></td>
                </tr>
                ...
                <tr>
                    <td colspan="2">
                        <button form="updateForm" type="submit" name="action" value="UpdateProfile" style="width: 100%">
                            Update Account
                        </button>
                    </td>
                    <td></td>
        ...
        <c:if test="${not empty errorReg && errorReg.clean eq false}">
                <span style="color: red">
                    Unable to update: <c:if test="${empty errorReg.otherError}">Invalid inputs</c:if>
                    <c:out value="${errorReg.otherError}"/>
                </span>
        </c:if>
    </div>
</div>
