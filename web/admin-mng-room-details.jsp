...
<c:set var="deleteError" value="${requestScope.DELETE_ERROR}"/>
    ...
    <div id="site_content">
        <div class="sidebar">
            ...
            <c:url var="updateLink" value="MainController">
                <c:param name="action" value="ForwardRoomUpdate"/>
                <c:param name="roomId" value="${room.id}"/>
                <c:param name="roomName" value="${room.name}"/>
            </c:url>
            <c:url var="deleteLink" value="MainController">
                <c:param name="action" value="DeleteRoom"/>
                <c:param name="roomId" value="${room.id}"/>
            </c:url>
            <a href="${updateLink}">Change Name</a>
            <br>
            <a href="${deleteLink}">Delete Room</a>
            <span style="color: red">${deleteError}</span>
        </div>

        <div id="content">
            <h1>${room.name}'s Personnel & Equipments</h1>
            <c:if test="${not empty room.userList}">
                <h2>Users List</h2>
                Total users: ${room.peopleCount}
                <table>
                    <tr>
                        <th>No</th>
                        <th>Username</th>
                    ...
                    <c:forEach var="user" items="${room.userList}" varStatus="counter">
                        <tr>
                            <td>${counter.count}</td>
                            <td>${user.username}</td>
                            ...
                            <td>
                                <c:choose>
                                    ...
                                    <c:when test="${user.role == 1}">Tech</c:when>
            ...
            <c:if test="${not empty room.equipmentList}">
                <h2>Equipment List</h2>
                Total Equipments: ${room.equipmentCount}
                <table>
                    <tr>
                        <th>No</th>
                        <th>ID</th>
                    ...
                    <c:forEach var="equipment" items="${room.equipmentList}" varStatus="counter">
                        <tr>
                            <td>${counter.count}</td>
                            <td>${equipment.id}</td>
                            ...
                            <td>
                                <c:choose>
                                    <c:when test="${equipment.status == 0}">Broken</c:when>
                                    <c:when test="${equipment.status == 1}">Good</c:when>
                                </c:choose>
                            </td>
