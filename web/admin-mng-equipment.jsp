    ...
    <div id="header">
        ...
        <div id="menubar">
            <ul id="menu">
                ...
                <li class="selected"><a href="admin-mng-equipment.jsp">Equipment</a></li>
    ...
    <div id="site_content">
        <div id="content">
                ...
                <table>
                    ...
                    <c:forEach var="equipment" items="${equipmentList}" varStatus="counter">
                        <tr>
                            ...
                            <td>
                                <c:if test="${equipment.status == 0 || equipment.status == 1}">
                                    <c:url var="transferLink" value="MainController">
                                        <c:param name="action" value="ForwardEquipmentTransfer"/>
                                        <c:param name="fromManagement" value="fromManagement"/>
                                        <c:param name="equipmentId" value="${equipment.id}"/>
                                        <c:param name="equipmentName" value="${equipment.name}"/>
                                        <c:param name="equipmentTypeName" value="${equipment.type.name}"/>
                                        <c:param name="equipmentOldRoom" value="${equipment.currentRoom.name}"/>
                                        <!-- SEARCH PARAMS ----->
                                        <c:param name="equipmentStatusSelect" value="${param.equipmentStatusSelect}"/>
                                        <c:param name="warrantySort" value="${param.warrantySort}"/>
                                        <c:param name="dateBoughtSort" value="${param.dateBoughtSort}"/>
                                    ...
                                    <a href="${transferLink}">
                                        <c:if test="${empty equipment.currentRoom.name}">
                                            <button style="width: 100%; height: 0.5cm">Assign</button>
                                        </c:if>
                                        <c:if test="${not empty equipment.currentRoom.name}">
                                            ${equipment.currentRoom.name}
                                        </c:if>
                                    </a>
                                </c:if>
                            </td>
            ...
            <table>
                <tr>
                    <td>
                        <form id="defaultSearchForm" action="MainController" method="post" style="margin-bottom: 0">
                            <table>
                                <tr>
                                    <th colspan="2" style="color: #207cff">Default</th>
                                </tr>
                                <tr>
                    ...
                    <td>
                        <form id="typeTwoSearchForm" action="MainController" method="post" style="margin-bottom: 0">
                            <table>
                                <tr>
                                    <th colspan="2" style="color: #207cff">On Repair Count</th>
                                </tr>
                                <tr>
                                    <th>Repair Count:</th>
                                    <td><input type="text" name="repairCount" value="${param.repairCount}"/></td>
                                </tr>
                ...
                <tr>
                    <td>
                        <button form="defaultSearchForm" type="submit" name="action" value="SearchEquipments"
                                style="width: 100%">
                            Begin Search
                        </button>
                    </td>
                    <td>
                        <button form="typeTwoSearchForm" type="submit" name="action"
                                value="SearchEquipmentsOnRepairCount"
                                style="width: 100%">Begin Search
                        </button>
                    </td>
                </tr>
                <tr>
                    <td colspan="2">
                        <form id="typeOneSearchForm" action="MainController" method="post" style="margin-bottom: 0">
                            <table>
                                <tr>
                                    <th colspan="2" style="color: #207cff">On Warranty Status & Expired Date Range</th>
                                </tr>
                                <tr>
                                    <th>Status</th>
                                    <td>
                                        <select name="equipmentStatusSelect">
                                            <option value="fine" ${param.equipmentStatusSelect == 'fine' ? 'selected' : ''}>
                                                Good
                                            </option>
                                            <option value="broken" ${param.equipmentStatusSelect == 'broken' ? 'selected' : ''}>
                                ...
                                <tr>
                                    <th>From (Date Expired):</th>
                                    <td><input type="date" name="dateExpiredFrom" value="${param.dateExpiredFrom}"/>
                                    </td>
                                </tr>
                                <tr>
                                    <th>To (Date Expired):</th>
                                    <td><input type="date" name="dateExpiredTo" value="${param.dateExpiredTo}"/></td>
                                </tr>
                            </table>
                        </form>
                    </td>
                ...
                <tr>
                    <td>
                        <button form="typeOneSearchForm" type="submit" name="action" value="SearchEquipmentsOnExpired"
                                style="width: 100%">Begin Search
                        </button>
                    </td>
                </tr>
            ...
            <c:url var="insertLink" value="MainController">
                <c:param name="action" value="ForwardEquipmentInsert"/>
                <!-- SEARCH PARAMS HERHE -->
                <c:param name="equipmentStatusSelect" value="${param.equipmentStatusSelect}"/>
                <c:param name="warrantySort" value="${param.warrantySort}"/>
                <c:param name="dateBoughtSort" value="${param.dateBoughtSort}"/>
